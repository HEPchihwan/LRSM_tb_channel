...
def twoak8cleancut(sample):
    ...
    etat = ak.sum(leading_toptagged_ak8["eta"], axis=1)
    etab = ak.sum(leading_bjet_ak8["eta"], axis=1)
    hlt = events["HLT_IsoMu30"].array()

    evt_mask = (
        (pt1 > 0) & (pt2 > 0) &
        (pt_leading_bjets > 0) & (pt_leading_topjets > 0) 
    )

    hlt_mask = (
        (pt1 > 30) & (pt2 > 0) &
        (pt_leading_bjets > 0) & (pt_leading_topjets > 0) & (hlt==True)
    ...
    lb  = leading_bjet_ak8[hlt_mask]
    lt  = leading_toptagged_ak8[hlt_mask]
    ...
    return combined_p4, mN, mu1_pt_flat, mu2_pt_flat,lt_flat,lb_flat, mll_flat ,lmi , slmi, lmhpt, slhpt ,totalev , leftev ,hltpassev 
...
def iter_allfile (path):
    ...
    for sample in tqdm(file_list, desc="Processing ROOT files"):
        ...
        combined_p4, mN, mu1_pt, mu2_pt,lt,lb, mll,lmi,lmh,smi,smh,totalev , leftev ,hltpassev = twoak8cleancut(sample)
...
lumi = 300
info = json.load(open("./WRtoNMutoMuMuTB-HadTop_MWR-3000_MN-2900_13p6TeV.json"))
print(info)
# 4) 계산 실행
